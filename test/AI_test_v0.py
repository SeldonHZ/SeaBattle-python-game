...
#  Battle v0 and awerage result for this test is 58.2 shots. I use such programs   #
...
def srch_cnt2(y) : #selection of the cell
    global dev
...
def main() : 
    global cord, play_cord_out, s1, lit,\
    chc_id, Data, dev, num, dest_by_comp, win, output, emp_space, \
    player_cord, play_input
    cycle = 10000 #amount of test cycles: the bigger amount --> the more accuracy is and the more time it will take to cuclulate result
    cyc = 0
    summation = 0
    mini = 100
    maxi = 0
        ...
        while win == 0 :
            ...
            while sank_cnd(play_input) : #computer move
                ...
                if hit_cnd(play_input) or prog > 0 :
                    if prog < 2 : #if the previous shot hit the ship, selects other cell near by
                        prog = 1; chc_id = []
                        srch_cnt1(prog)
                        srch_cnt2(prog)
